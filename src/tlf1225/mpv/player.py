...
def setup(func):
    ...
    def log_mpv(loglevel, component, message) -> None:
        ...
        log.write(f"[{loglevel}] {component}: {message}{linesep}")
        if log.tell() > 16384:
            log.seek(0)
            print(linesep.join(set(log.readlines())), file=stderr)
            log.truncate(0)
            log.seek(0)
    ...
    def event(func2):
        def temp():
            for i in [z for z in [getattr(func2, y) for y in dir(func2) if y.startswith("un")] if callable(z)]:
                i()
            event_handler.remove(func2)

        func2.clear = temp

        event_handler.append(func2)
    ...
    def wrapper(*args, **kwargs):
        sleep(1)
        kwargs.update({"player": player, "event": event_handler, "log": log})
        func(*args, **kwargs)
        for i in event_handler.copy():
            i.clear()
        player.quit()
...
def main(*args, **kwargs):
    ...
    event_handler = kwargs.get("event")
    url = kwargs.get("url")
    if not isinstance(player, MPV) or not isinstance(event_handler, list) or not url:
        return
...
if __name__ == '__main__':
    ...
    if len(argv) > 1:
        ...
        main(url="ytdl://PLfwcn8kB8EmMQSt88kswhY-QqJtWfVYEr")
